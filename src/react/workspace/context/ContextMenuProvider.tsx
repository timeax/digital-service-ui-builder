import { MenuRenderer } from '@/components/context-menu/MenuRenderer';
...
export type ContextScope = 'file' | 'folder' | 'library' | 'empty' | (string & {});
...
export interface MenuItem {
    ...
    onSelect?: (args: { ctx: CtxWithAttach; nativeEvent: MouseEvent | KeyboardEvent }) => void | Promise<void>;
    children?: MenuItem[];
}
...
export type MenuBuilder = MenuItem[] | ((opts: { ctx: ContextState; nativeEvent?: MouseEvent }) => MenuItem[] | Promise<MenuItem[]>);
...
export interface ContextMenuApi {
    openAt: (evOrCoords: MouseEvent | { x: number; y: number }, items: MenuBuilder, options?: OpenOptions) => void;
    close: () => void;
...
export const useContextMenu = (): ContextMenuApi => {
    ...
    if (!api) {
        throw new Error('useContextMenu() must be used under <ContextMenuProvider>.');
    }
    return api;
};
...
export const ContextMenuProvider: React.FC<{ children: React.ReactNode; zIndex?: number }> = ({
    children,
    ...
    const resolveItems = useCallback(async (items: MenuBuilder, opts: OpenOptions): Promise<MenuItem[]> => {
        let raw: MenuItem[] | Promise<MenuItem[]>;
        if (typeof items === 'function') {
            //@ts-expect-error
            raw = items({ ctx: opts.ctx ?? {}, nativeEvent: opts.nativeEvent });
        } else {
            raw = items;
        }
    ...
    const openAt = useCallback<ContextMenuApi['openAt']>(
        async (evOrCoords, items, options) => {
            const nativeEvent = 'x' in evOrCoords ? options?.nativeEvent : (evOrCoords as MouseEvent);
            const coords = 'x' in evOrCoords ? evOrCoords : { x: (evOrCoords as MouseEvent).clientX, y: (evOrCoords as MouseEvent).clientY };

            if (!('x' in evOrCoords) && (evOrCoords as MouseEvent).preventDefault) {
                (evOrCoords as MouseEvent).preventDefault();
            }

            const resolved = await resolveItems(items, { ...(options ?? {}), nativeEvent });
            setState({
                open: true,
                x: coords.x,
                y: coords.y,
            ...
            return (ev: React.MouseEvent) => {
                ...
                Promise.resolve(maybePromise).then((items) => {
                    openAt(
                        ...
                        typeof items === 'function' || Array.isArray(items) ? (items as MenuBuilder) : [],
    ...
    const api = useMemo<ContextMenuApi>(() => ({ openAt, close, bind }), [openAt, close, bind]);
                ...
                createPortal(
                    ...
                    >
                        <MenuRenderer
                            ...
                            ctx={state.ctx}
                            onClose={close}
                            onAction={(it, e) => it.onSelect?.({ ctx: state.ctx, nativeEvent: e })}
                        />
                    </div>,
                    document.body,
                )}
