                    ...
                    <li key={node.id}>
                        ...
                        >
                            ...
                            <span className="text-xs text-muted-foreground">
                                {
                                    fields.filter((f) =>
                                        Array.isArray(f.bind_id)
                                            ? f.bind_id.includes(node.id)
                                            : f.bind_id === node.id,
                                    ).length
                                }
                            </span>
            ...
            <div className="px-3 pb-3 overflow-auto">
                ...
                {isMulti ? (
                    <div className="text-sm text-muted-foreground">
                        Workspace multi-select active. Use canvas actions to
                        edit multiple groups.
                    </div>
                ) : (
                    <ul className="space-y-1">
                        {(group?.fields ?? []).map((f) => {
                            const bound = Array.isArray(f.bind_id)
                                ? f.bind_id
                                : [f.bind_id].filter(Boolean);
                            const isSrv = (f.options ?? []).some(
                                (o) => o.service_id != null,
                            );
                            return (
                                <li
